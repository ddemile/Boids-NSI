...
C'est une simulation de boids (contraction de bird-oid : qui a la forme d'un oiseau) interactive qui a pour but de permettre à l'utilisateur de simuler le comportement d'une nuée d'oiseaux en vol tout en permettant de changer différent paramètres.

## Présentation de l'équipe
Notre équipe est composé de 4 élèves de première au Lycée Moulin Joli à La Possession. Voici comment la répartition des tâches s'est effectuée :
...
7. Equilibrage afin de trouver de bonnes valeurs de départ
...
Notre projet étant une simulation, l'ajout de tests automatiques aurait été assez difficile. Pour pallier à ce manque, nous avons testés toutes les fonctionnalités du projet avant de le rendre pour s'assurer qu'il n'y ait pas de bugs (ou presque pas).

### Difficultés rencontrées
Durant la phase de développement, un problème majeur est apparu : l'optimisation. En effet, les premières versions du projet étaient presque inutilisables à cause du nombre d'images par seconde très faible. N'arrivant pas à trouver de solution, nous avons demandés de l'aide à ChatGPT qui nous a proposé d'implémenter un système de grille à l'aide d'une fonction : `build_grid`, nous avons par la suite modifié cette fonction pour qu'elle puisse s'adapter à notre projet.

## Ouverture
### HOARAU Emilien
Le projet **Boid's Night** m'a permis d'en apprendre le plus sur le module pyxel. J'aurais aimé pouvoir simuler un nombre plus important de boids, mais nous n'avons pas réussi à optimiser le projet plus que dans son état actuel.
...
En regardant le projet après coup, je trouve que nous avons pu réaliser un projet concret et le finir. Cependant, je pense que nous aurions pu ajouter d'autres fonctionnalités, comme la possibilité de faire suivre le curseur aux boids ou même de pouvoir choisir entre différents thèmes.
...
### DIOT Eloïse
La réalisation de notre projet Boid's night à été une expérience très enrichissante !
...
J’ai pu progresser en  codage, amélioré ma capacité à travailler en équipe et à m’auto-organiser.
Au niveau du projet, d'autres améliorerations ont été évoqué, j'aurais par exemple aimé pouvoir créer un arrière-plan animé mais notre projet reste une belle réussite dont nous sommes fiers et qui mérite une note conséquente !

### WATANABE Romain
Quand on a travaillé sur le projet, j'ai eu plusieurs idées qui venaient, comme par exemple faire des centaines de mini oiseaux de milliers de couleurs différentes. Ce projet m'a permis de m'entraîner à mieux m'exprimer à l'oral.
De mon point de vue, le jeu est très bien fait, même s'il pourrait encore être amélioré dans le futur.