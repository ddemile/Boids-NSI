...
C'est une simulation de boids (contraction de bird-oid : qui a la forme d'un oiseau) interactive qui a pour but de permettre à l'utilisateur de simuler le comportement d'une nuée d'oiseaux en vol tout en permettant de modifier différent paramètres.
...
Notre projet étant une simulation, l'ajout de tests automatiques aurait été assez difficile. Pour pallier à ce manque, nous avons testé toutes les fonctionnalités du projet avant de le rendre pour s'assurer qu'il n'y ait pas de bugs (ou presque pas).
...
## Ouverture
### HOARAU Emilien
Le projet **Boid's Night** m'a permis d'en apprendre davantage sur le module pyxel. J'aurais aimé pouvoir simuler un nombre plus important de boids, mais nous n'avons pas réussi à optimiser le projet plus que dans son état actuel.
...
En regardant le projet après coup, je trouve que nous avons pu réalisé un projet concret et le finir. Cependant, je pense que nous aurions pu ajouter d'autres fonctionnalités, comme la possibilité de faire suivre le curseur aux boids ou même de pouvoir choisir entre différents thèmes.
